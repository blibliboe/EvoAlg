...
def get_variation_fn(
    ...
    max_generations: int  # New parameter to keep track of maximum generations
):
    @nb.jit((
        ...
        nty.int32,  # generation
        nty.int32   # max_generations
        ), nopython=True, nogil=True, fastmath={"nsz", "arcp", "contract", "afn"}, error_model="numpy", cache=True, parallel=False)
    def perform_variation(structures, constants, fitness, trial_structures, trial_constants, trial_fitness, X, y, rng, generation, max_generations):
        ...
        for i in range(population_size):
            ...
            # Time-dependent crossover probability
            ...
            time_dependent_crossover = lambda j: (0.15 - 0.1 * (generation / max_generations)) if j < max_expression_size // 2 else (0.05 + 0.1 * (generation / max_generations))

            # Construct trial population
            for j in range(structures.shape[1]):
                if rng.random() < time_dependent_crossover(j) or j == j_rand:
